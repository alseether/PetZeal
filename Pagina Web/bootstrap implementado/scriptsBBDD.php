...
function actualizaFotoUsuario($idUsuario, $imagen){
	global $mysqli;
	$query="UPDATE usuarios SET  Imagen='".$imagen."'
	WHERE IDusuario='".$idUsuario."'"; 
	$ret = $mysqli->query($query) or die ($mysqli->error. " en la linea".(__LINE__-1));
}
...
function actualizaFotoMascota($idMascota, $imagen){
	global $mysqli;
	$query="UPDATE mascotas SET  Imagen='".$imagen."'
	WHERE IDmascota='".$idMascota."'"; 
	$ret = $mysqli->query($query) or die ($mysqli->error. " en la linea".(__LINE__-1));
}
